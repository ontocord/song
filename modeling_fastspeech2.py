...
class VarianceAdaptor(nn.Module, ModuleUtils):
    """ Variance Adaptor """
...
class PositionwiseFeedForward(nn.Module):
    ...
    def __init__(self, d_in, d_hid, config, d_out=None):
        super().__init__()
...
class Encoder(nn.Module, ModuleUtils):
    ''' Encoder '''
    ...
    def forward(self, src_seq, mask, return_attns=False, labels=None, do_char_to_phoneme=False):
        device = next(self.parameters()).device
        ...
        if src_seq.shape[1] > self.max_seq_len:
            ...
            position = get_sinusoid_encoding_table(src_seq.shape[1], self.encoder_hidden)[:src_seq.shape[1], :].unsqueeze(0).expand(batch_size, -1, -1).to(enc_output.dtype).to(device)
            position.requires_grad = False
            enc_output = enc_output + position
        else:
...
class FastSpeech2(FastSpeech2PreTrainedModel, ModuleUtils):
    """ FastSpeech2 """
    ...
    def forward(self, src_seq, attention_mask, return_wav_list=True, mel_len=None, d_target=None, p_target=None, e_target=None, max_src_len=None, max_mel_len=None, log_d_target=None, use_postnet=None, labels=None, do_char_to_phoneme=False):
        ...
        encoder_output = self.encoder(src_seq, src_mask, labels=labels, do_char_to_phoneme=do_char_to_phoneme)
    ...
    def freeze_generator(self):
        """
        Calling this function will disable the gradient computation for the generator so that its parameter
        will not be updated during training.
        """
        self.generator._freeze_parameters()

    def freeze_variance_adaptor(self):
        """
        Calling this function will disable the gradient computation for the variance_adaptor so that its parameter
        will not be updated during training.
        """
        self.variance_adaptor._freeze_parameters()
...
class FastSpeech2ForPretraining(FastSpeech2PreTrainedModel):
    ...
    def freeze_generator(self):
      self.fastspeech2.freeze_generator()
